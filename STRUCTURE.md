...
estimator in its own fieldset below Tag Dimensions — always-visible
reliable readout with camera preset, edit gated behind “Set tag size from
distance” checkbox. Does not touch layout or rendering.
...
| `src/detection/range.ts` | Pure detection-range estimator: `focalLengthPx` (pinhole from HFOV + width), `estimateRange` (reliable distance for the family's `DetectionModel`), and `minTagSizeForDistance` (inverse, rounded up to 0.5 mm) for the checkbox-gated distance → tag-size sync. Loud on invalid input. |
